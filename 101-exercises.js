...
for (let i =0;i<fruits.length;i++){
  fruitsAndVeggies.push(fruits[i]);
}
for (let j=0;j<vegetables.length;j++) {
  fruitsAndVeggies.push(vegetables[j]);
}
...
function isPositive(number){
  ...
  if (Math.sign(number) === -0){ return false }
    if (Math.sign(number) === 0){ return false }
}
...
function isPositiveOdd(number){
  if ((number % 2 !== 0) && (Math.sign(number) === 1) )
  {return true
  }else{return false}
};
...
function hasVowels(word){
  ...
  if (proof > 0){return true}else {return false}
}
...
function firstAndLast(sequence){
    let result = [sequence[0],sequence[sequence.length-1]]
    return result;
}
...
function firstToLast(sequence){
    let tmp = sequence[sequence.length-1];
